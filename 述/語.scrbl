...
@述機[(引-機-1 [甲 引-機？]) (-> 映？ 未算？ ... 物？)]{@語[引-機]的反函數。}
...
@section{誤}
@subsection{停機}
實現應該識別出儘量多的停機，改爲@語[(誤 (構 {引 誤/界/停} ()))]。
@subsection{內置}
內置的機產生的@語[誤]是@語[(誤 (構 {引 誤/界} (列 -名 -式 -其他)))]。@語[-名]是它的名稱，@語[-式]是它的參數，@語[-其他]一般是@語[()]。

內置特殊形式的@語[誤]是@語[(誤 (構 {引 誤/界/式} (列 -名 -境 -式 -其他)))]。@語[-名]是它的名稱，@語[-境]是環境，@語[-式]是它的參數，@語[-其他]一般是@語[()]。
