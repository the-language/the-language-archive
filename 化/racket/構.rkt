...
{provide
 ...
 列.？ 列.連.？ 列.連.建+ 列.連.首+ 列.連.尾+ 列.空.？ 列.空.其 列.建+
 名.？ 名.文.？ 名.構.？ 名.構.建+ 名.構.:名+ 名.構.:列+
 表.？ 表.空 表.增+ 表.改+ 表.取+ 表.含？ 表.删+ 表.→列
 集.？ 集.空 集.增+ 集.含？ 集.删+ 集.→列 集.建+
 化.？ 化.建+ 化.形+ 化.:物+ #%化.內.建+ #%化.內+
 式.？ 式.建+ 式.反+
 構.？ 構.建+ 構.:名+ 構.:列+
 誤.？ 誤.建+ 誤.反+
 }
...
{define (集.建+ . xs)
  (if (列.空.？ xs)
      集.空
      (集.增+ (apply 集.建+ (列.連.尾+ xs)) (列.連.首+ xs)))}

{define-rec 化.？ (#%化.內.建+ #%化.內+ 化.形+ 化.:物+)}
{define (化.建+ 形 :物) (#%化.內.建+ #f 形 :物)}

{define-rec 式.？ (式.建+ 式.反+)}
{define-rec 構.？ (構.建+ 構.:名+ 構.:列+)}
{define-rec 誤.？ (誤.建+ 誤.反+)}
