...
@section{誤}
@subsection{無}
替換：在進行0或+更多次替換後，可以把沒有結果（需要無限的時間的）的任意個@語[:物]替換爲@語[(→誤 (→構 {引 誤/界/無} <未定義>))]，@語[<未定義>]是任意的@語[:物]。

實現應該儘量避免無限的運行下去。

實現應該儘量避免無用的替換。
@subsection{化}
內置的任何@語[:化]產生的@語[:誤]是@語[(→誤 (->:構 {引 誤/界/化} ((化.建+ '(:列) ':列) <:名> <式> <位:名>)))]，
