...
@section{形式}
@述[(#%式 式 甲 ...)]{等價與使用@語[化.用]和@語[式->]。使用一個@語[:式？]@語[式]。可以寫作@語[{式 甲 ...}]。}
@述值[頂:表 :表？]
@述[(#%頂 名)]{頂層的物。}
@述[(化 形 ...)]{等價與使用@語[化.用]。}
...
@subsection{述}
@述化[(等？ [甲 (非 :誤？)] [乙 (非 :誤？)]) :<集_陰_陽>？]{返回@語[甲]是否等於@語[乙]。}
@述化[(算 [:物 物.？] [:表 :表？]) 物.？]
@述化[(取 [:名 名？]) 物.？]{獲取一個包}
...
@subsubsection{陰-陽}
@述化[(:<集_陰_陽>？ [:物 (非 :誤？)]) :<集_陰_陽>？]
@述化[(:陰？ [:物 (非 :誤？)]) :<集_陰_陽>？]
@述值[其:陰 :陰？]
@述化[(:陽？ [:物 (非 :誤？)]) :<集_陰_陽>？]
@述值[其:陽 :陽？]
@述化[(<集_陰_陽>.若 [:<集_陰_陽> :<集_陰_陽>？] [甲 物.？] [乙 物.？]) 物.？]{若@語[:<集_陰_陽>]是@語[其:陽]，則返回@語[乙]，否則返回@語[丙]。}
...
實現應該儘量避免無用的替換。
@subsection{化}
內置的任何@語[:化]產生的@語[:誤]是@語[(誤.建 (構.建 {!引 誤/界/化} ((化.建 '(:列) ':列) <:名> <式> <位:名>)))]，
